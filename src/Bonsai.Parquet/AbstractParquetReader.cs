...
namespace Bonsai.Parquet
{
    ...
    /// per row. Subclass to declare which columns to read and how to assemble rows.
    /// </summary>
    [Combinator(MethodName = nameof(Generate))]
    [WorkflowElementCategory(ElementCategory.Source)]
    public abstract class AbstractParquetReader<TRow>
    {
        ...
        /// and return a factory from row index → <typeparamref name="TRow"/>. The arrays remain
        /// valid until the next call, so the closure may capture them safely.
        /// </summary>
            ...
            return Observable.Create<TRow>(observer =>
            {
                ...
                Task.Run(() =>
                {
                    try
                    {
                        ...
                        // ColumnRoot gives the top-level field name; for LIST columns the leaf is
                        // named "item" but the user-visible name lives on the root node.
                        ...
                        // Validate types via row group 0; skipped for empty files where no
                        // type mismatch can occur at runtime.
